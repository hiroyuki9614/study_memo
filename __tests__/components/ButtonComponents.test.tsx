...
describe('Button Component', () => {
	...
	// サイズバリエーションのテスト
	it.each(['xs', 'sm', 'base', 'lg', 'xl'] as const)('サイズ %s が正しく適用される', (size) => {
	...
	// バリアントのテスト
	it.each(['primary', 'secondary', 'tertiary', 'disable'] as const)('バリアント %s が正しく適用される', (variant) => {
	...
	it('アイコンの位置が正しく設定される', () => {
		render(
			<Button icon={faSearch} iconPosition='right'>
				Search
			</Button>
		);
		const button = screen.getByRole('button');
