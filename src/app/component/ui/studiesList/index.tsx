...
const ReadAllStudiesData: React.FC<MonthlyStudiesProps> = ({ allStudiesData, year }) => {
	...
	return (
		<article className='flex items-center box-border flex-col justify-center'>
			<div className='bg-slate-50 rounded-2xl shadow shadow-slate-500 mb-2 p-3 w-10/12' onClick={handleYearClick}>
				<h2 className=''>{year}年の学習内容</h2>
			</div>
			{isOpenYear &&
				sortedMonths.map((month) => (
					<div key={month} className='bg-slate-50 rounded-2xl shadow shadow-slate-500 mb-2 p-4 w-10/12'>
